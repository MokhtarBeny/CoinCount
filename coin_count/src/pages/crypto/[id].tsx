...
export default function CryptoPage() {
  ...
  return (
    <>
      <div className="md:m-12 gap-2 grid grid-cols-12 grid-rows-2 md:px-8 m-4">
        <Card
          isFooterBlurred
        ...
        >
          <CardHeader className="absolute z-10 top-1 flex justify-between ">
            <div>
              <p className="text-tiny text-indigo-400 uppercase font-bold">
                {crypto.symbol}
              </p>
              <h4 className="text-black font-medium text-2xl">{crypto.name}</h4>
            </div>
          ...
          <CardBody className="justify-center items-center">
            <h4 className="text-indigo-800 font-bold text-2xl">
              {crypto.rank}
            </h4>
          ...
          <CardFooter className="absolute bg-white/30 bottom-0 border-t-1 border-zinc-100/50 z-10 justify-between">
            <Button
              ...
              size="sm"
            >
              <a href={crypto.explorer}>Website</a>
            </Button>
            <Button
              className="text-tiny"
              color="primary"
              ...
              size="sm"
            >
              <a href={crypto.explorer}>Explorer</a>
            </Button>
          </CardFooter>
        </Card>
        ...
        <Card className="col-span-12 sm:col-span-5 h-full  items-center  justify-center content-center flex flex-row ">
          <CardBody className="flex">
            <p className="text-tin text-indigo-400 font-bold">Volume (24h)</p>
            <h4 className=" font-medium text-large">
              {parseFloat(crypto.volumeUsd24Hr).toFixed(2)}$
            </h4>
            <p className="text-tin  font-bold text-indigo-400">VWap 24h</p>
            <h4 className=" font-medium text-large">
              {parseFloat(crypto.vwap24Hr).toFixed(2)}$
            </h4>
          </CardBody>
          <CardBody className="flex ">
            <div className="">
              {" "}
              <p className="text-tin  font-bold text-indigo-400">Market Cap</p>
              <h4 className=" font-medium text-large ">
                {parseFloat(crypto.marketCapUsd).toFixed(2)}$
              </h4>
              <p className="text-tin  font-bold text-indigo-400">Supply</p>
              <h4 className=" font-medium text-large">
                {parseFloat(crypto.supply)} {crypto.symbol}
              </h4>
            </div>
          </CardBody>
        </Card>
